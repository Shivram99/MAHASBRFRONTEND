...
@Component({
  selector: 'app-duplicate-brn-details',
  standalone: false,
  templateUrl: './duplicate-brn-details.component.html',
  styleUrl: './duplicate-brn-details.component.css'
})
export class DuplicateBrnDetailsComponent {
  ...
  ngOnInit(): void {
    ...
    this.loadDuplicateDetails(this.currentPage, this.pageSize, this.sortBy);

  }

  loadDuplicateDetails(page: number, size: number, sortBy: string):void{
    this.fileUploadService.getDuplicateDetailsPage(page, size, sortBy).subscribe(
    (response: PaginatedResponse<MstRegistryDetailsPage>) => { // Expecting an array of MstRegistryDetailsPage
      this.deuplicateDetails = response.content;
